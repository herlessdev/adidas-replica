...
const Footer = ({ dictionary }: Props) => {
  return (
    <footer className="mt-auto">
      <div className="flex lg:flex-row flex-col lg:gap-0 gap-8 bg-[#ede734] text-center py-[37px] items-center justify-between uppercase font-bold px-4 sm:px-16">
      ...
      <div className="bg-[#282c31] pt-8 pb-4 flex flex-col gap-8 text-[#c2c4ca] text-xs">
        <ul className="grid grid-cols-2 px-4 sm:px-16 lg:px-0 lg:flex flex-wrap gap-2 sm:gap-4 items-center justify-center">
          {dictionary?.finished?.opts?.map((opt: any, i: any, arr: any) => (
            <React.Fragment key={i}>
              <li className="self-start px-5">{opt}</li>
              {i !== arr.length - 1 && (
